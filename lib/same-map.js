...
module.exports = function sameMap(a, b, opts, memos) {
  ...
  if (set.size > 0) {
    for (const [key, item] of b) {
      ...
      if (a.has(key) && same(a.get(key), item, { strict: false }, memos))
        continue

      if (mapHasEqualEntry(set, a, key, item, { strict: false }, memos))
        continue
...
function mapHasEqualEntry(set, a, key, bValue, opts, memos) {
  for (const key2 of set) {
    if (
      same(key, key2, opts, memos) &&
      same(bValue, a.get(key2), opts, memos)
    ) {
      set.delete(key2)
      return true
    }
